...
document.onkeydown = function(e) {
    ...
    if (common.ismode('change') && main.isview === true) {
        ...
        if ((common.keyStatus[key.c] === true) && (e.metaKey === true)) {
            main.movefrom = main.workdir.find(".row_item:contains('" + main.name + "')");
    ...
    if (common.ismode('change') && main.isview === false) {
        if ((common.keyStatus[key.v] === true) && (e.metaKey === true)) {
            common.setmode('move');
            const moveto = main.workdir.find('.change');
            main.setElementData(moveto, function(elm) {
                main.name = elm.find('.row_item').text();
            },{cliped: main.cliped});
            const dataSet = main.getElementData();
            const done = function(response) {
                const add = $('<div class="row"><p class="row_item">' + response + '</p></div>');
    ...
    if (common.ismode('edit') && (main.isview === false)) {
        if ((common.keyStatus[key.ctrl] === true) && (common.keyStatus[key.c] === true)) {
            $edit.trigger('click');
        }
    }
};
